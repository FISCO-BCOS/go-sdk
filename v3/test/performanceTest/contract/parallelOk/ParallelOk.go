...
func DeployParallelOk(auth *bind.TransactOpts, backend bind.ContractBackend) (common.Address, *types.Receipt, *ParallelOk, error) {
	...
	address, receipt, contract, err := bind.DeployContract(auth, parsed, common.FromHex(ParallelOkBin), ParallelOkABI, backend)
	if err != nil {
		return common.Address{}, nil, nil, err
	}
...
func AsyncDeployParallelOk(auth *bind.TransactOpts, handler func(*types.Receipt, error), backend bind.ContractBackend) (*types.Transaction, error) {
	...
	tx, err := bind.AsyncDeployContract(auth, handler, parsed, common.FromHex(ParallelOkBin), ParallelOkABI, backend)
	if err != nil {
		return nil, err
	}
	return tx, nil
